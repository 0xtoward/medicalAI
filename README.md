...
### 数据与方法
研究基于 `1003` 条治疗记录、`889` 名患者的纵向随访数据，随访时间点为 `0M`、`1M`、`3M`、`6M`、`12M`、`18M`、`24M`。在外层验证中，全部数据按患者首次出现顺序进行 `80/20` 时序切分，得到训练集 `711` 名患者、`795` 条记录，测试集 `178` 名患者、`208` 条记录。核心变量包括 `FT3`、`FT4`、`TSH`、医生诊断状态以及多项基线临床特征。模型开发采用时序安全 MissForest 填补、患者分组切分、`GroupKFold` 内部验证和 OOF 阈值选择。固定 Landmark 模块以 `3M` 和 `6M` 为预测时点；滚动 Landmark 模块使用 pooled long-format 数据预测 `Normal -> Hyper` 复发。
...
## 数据来源与变量
当前数据集包含 `1003` 条治疗记录和 `889` 名唯一患者。随访时间点包括 `0M`、`1M`、`3M`、`6M`、`12M`、`18M`、`24M`。全部分析共用同一外层时序切分：训练集 `711` 名患者、`795` 条记录，测试集 `178` 名患者、`208` 条记录。其中：
...
- 外层数据按患者首次出现顺序进行 `80/20` 时序切分：训练集 `711` 名患者、`795` 条记录；测试集 `178` 名患者、`208` 条记录
- 训练/测试切分先于任何预处理步骤
- MissForest 仅在训练集上拟合
...
需要说明的是，滚动 Landmark 复发分析并不是对全部 `889` 名患者直接建模，而是仅纳入“在某一随访节点处于 `Normal`、因此具备 `Normal -> Hyper` 复发预测意义”的患者-区间。故其 long-format 区间数和后续 patient-level 有效患者数会小于总体外层切分人数。
...
### Patient-level 结果
在 patient-level 汇总后，纳入的是动态复发分析子队列，而非全部外层切分患者：
