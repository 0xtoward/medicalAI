...
- 纳入队列：`889` 个患者（其中 `109` 人曾接受两次放射性碘治疗，共贡献 `1003` 条治疗记录）
...
全队列共 `889` 人。由于本研究的复发定义为 **Normal → Hyper 的状态回转**，只有在某一随访时点处于正常甲功状态（Normal）的患者才被视为"处于复发风险中"（at risk），其对应的随访区间才会进入模型的训练或评估。

在 `889` 人中，有 `229` 人在全部随访窗口内均未进入正常甲功状态——他们在治疗后始终处于甲亢或甲减状态，不满足 Normal → Hyper 复发的前提条件。这些患者并非被主观排除，而是在构建纵向 at-risk 区间时自然不产生任何预测行；在患者级汇报中，他们的**累积复发风险统一记为 `0`、复发观察统一记为 `0`**，因为他们从未暴露于 Normal → Hyper 的风险路径。

- **全队列**：`889` 人
  - 开发期（前 `80%`）：`711` 人
  - 时间外推测试集（后 `20%`）：`178` 人
- 其中贡献 at-risk 区间的患者：`660` 人
  - 开发期：`529` 人（贡献区间用于训练和内部验证）
  - 时间外推测试集：`159` 人（贡献区间用于测试）

时间顺序切分在**全队列层面**完成（即先按入组顺序将全部 `889` 人分为 `711 / 178`），之后再在纵向表构建阶段自然过滤出 at-risk 区间。这一顺序确保了分组边界不受 Normal 状态出现与否的影响，避免引入未来信息偏倚。**下文所有患者级图表均以全队列口径汇报**（开发期 `711` 人、测试集 `178` 人），不再拆分出"分析队列"子口径。唯一例外是 §3.4 的 fixed-landmark 分析：由于 `109` 位患者曾接受两次放射性碘治疗，该分析以治疗记录为单位（共 `1003` 条），以充分利用同一患者不同治疗轮次的早期反应信息。
...
下列细节并非单独的算法贡献，但每一项都直接影响模型的可靠性、可复现性或临床可接受度。对熟悉统计学习的读者可快速跳过，对临床背景的审稿人则建议浏览本节以了解"为什么这些技术选择很重要"。

#### 2.4.1 缺失值处理：MissForest 多变量条件插补

临床随访数据中不可避免地存在缺失——某次未抽血、某项化验单丢失、某个窗口未按时复查。传统做法通常是用中位数或均值填充，但这会把所有缺失患者的化验值"拍平"到同一个数字上，抹去了个体差异。

本研究采用 **MissForest**（基于随机森林的迭代条件插补）：对每个缺失变量，用其余**所有未缺失变量**作为预测因子，训练一棵随机森林来预测该变量最可能的取值，然后迭代多轮直到收敛。这意味着：

- 一位甲状腺重量偏大、TRAb 偏高的患者，其缺失 FT4 的插补值会偏向同类"高基线负担"患者的 FT4 分布，而非全队列均值；
- 有序分类变量（如甲功状态 Hyper / Normal / Hypo）在插补后会被四舍五入回合法的离散取值，不会产生"2.3 = 介于正常和甲减之间"这种无临床意义的结果。

关键的泄漏防护：MissForest 仅在**开发期（711 人）**上拟合，测试集患者的缺失值使用开发期学到的条件分布进行填充，不会反向接触测试集信息。

#### 2.4.2 患者级分组交叉验证（GroupKFold）

在开发期内部进行模型选择和超参数调优时，本研究使用 **GroupKFold**——以**患者**（而非随访记录）为分组单位进行交叉验证。这意味着同一患者的所有随访窗口只能同时出现在训练折或验证折中的一个，不会出现"模型在第 3 折见过张三的 3M 数据，在第 4 折被问张三的 6M 数据"这种隐性信息泄漏。

这一设计与纵向医学数据的特点匹配：同一患者不同窗口之间存在强自相关，如果允许拆开分配，模型可以通过记住患者特异性模式而非学习通用规律来获得虚高的验证分数。

#### 2.4.3 Out-of-Fold 预测与知识蒸馏

模型训练中有两处用到了 **out-of-fold (OOF) 预测**——一种"避免自己给自己打分"的预测生成方式：

1. **基础动态风险头的 soft label**：先训练一个传统强基线模型（如弹性网络逻辑回归），然后对训练集中每个样本生成其 OOF 预测概率。所谓 OOF，就是在 K 折交叉验证中，每个样本只使用**不包含自己的那 K-1 折**训练出的模型来给自己打分。这样得到的概率既包含了强基线的排序信息，又不存在"模型在训练数据上过度自信"的偏差。
2. **3M landmark 头的 soft target**：fixed-landmark 强模型的 OOF 概率被用作 3M 头的连续回归目标，替代硬二元标签（是 / 否甲亢），让早期反应信号在共享表征中保持更细腻的梯度信息。

两处 OOF 的共同作用是实现**知识蒸馏**（knowledge distillation）：让最终的深度模型不仅学习原始标签，还吸收传统强模型已经提炼出的排序经验，同时避免因直接使用训练集预测值而引入过拟合偏差。

#### 2.4.4 时间安全的特征工程

所有动态特征严格遵守**时间因果约束**：在每个随访 landmark `t`，模型只能看到 `t` 及之前的信息。具体而言：

- `Delta_FT4_1step` 只使用当前窗口与前一窗口的差值，不会偷看后续窗口的变化趋势；
- `Time_In_Normal` 是截至当前的累计值，不是全病程事后统计；
- `Prior_Relapse_Count` 是截至当前的既往复发次数，不包括当前窗口或未来窗口的复发事件。

这一约束确保模型在临床部署时可以在每次复查后**实时更新风险**，而不是依赖只有回顾性研究才能获得的"全部随访结束后的完整信息"。

#### 2.4.5 冻结读出层：从"黑箱得分"到"可审计决策"

最终部署的风险并非由深度网络直接输出。训练完成后，整个 backbone 被**冻结**（参数不再更新），最终风险由一个仅有 4 个输入的**逻辑回归**给出（详见 §5.1）。这意味着：

- 任何一位患者的最终风险得分都可以被**精确追溯**到四个可命名的信号维度；
- 医生无需理解神经网络内部结构，只需要知道"风险主要由基础动态得分驱动，受早期反应信号和近期 / 基线信息权重调节"；
- 如果某位患者的风险评估引起临床疑问，可以直接查看这四个输入值来定位原因，而不是面对一个不可解释的 0.37。

#### 2.4.6 Wilson 置信区间与描述性分层

在患者级风险分层（§4.7）中，每个四分位组的观察复发率配有 **Wilson 95% 置信区间**而非简单的正态近似区间。Wilson 区间的优势在于：当事件率接近 0 或 1、样本量较小时，它不会产生负数或超过 1 的荒谬边界——这在医学小样本亚组分析中尤其重要。
...
该分析以**治疗记录**为单位（`889` 位患者的 `1003` 条治疗记录，其中开发期 `795` 条、时间外推测试 `208` 条），在 `3M` 时点预测患者是否仍处于甲亢状态：

![固定 landmark 性能热图](results/fixed_landmark_binary/Performance_Heatmaps.png)

- 最佳时间外推测试 `AUC = 0.857`（`Elastic+LGBM Blend`）
- 最佳时间外推测试 `Accuracy = 0.813`（`Elastic+LGBM Blend Routed`）
- 最佳时间外推测试 `F1 = 0.752`（`Elastic+LGBM Blend Routed`）
...
在患者级分析中，同一病人的多窗口风险按 `1 - ∏(1 - p_t)` 聚合为**累计患者级风险**。对于从未进入 Normal 状态的患者（无 at-risk 区间），累积风险统一记为 `0`。

四分位分层直接在时间外推测试集（`N = 178`）上定义，每组各约 `25%`（`45 / 44 / 44 / 45`）。`Q4` 的观察到复发率最高，且均值预测风险也最高。图中蓝柱表示各组真实复发率，橙线表示各组平均预测风险；柱顶标注采用 `复发人数/组内总人数`，误差条为 Wilson `95%` 置信区间。

需要谨慎的是：`95% CI` 反映的是各组观察到复发率本身的不确定性，而不是组间差异的正式显著性检验。`Q4` 风险负担最高这一点清楚，但中间风险层之间仍存在区间重叠，因此它更适合被理解为**描述性风险分层图**。
...
## 附录 A. 6M 固定 Landmark 二分类补充结果

§3.4 聚焦于 `3M` 时点的早期反应信号。实际上，相同的 fixed-landmark binary 分析也在 `6M` 时点进行了完整评估（同样以 `889` 位患者的 `1003` 条治疗记录为基础）。`6M` 信号在判别力上甚至更强，但因为它距治疗已过半年，对"早期分诊"的直接指导价值不如 `3M`，因此作为补充材料保留。

### A.1 6M 时间外推测试集性能

| 模型 | AUC | Accuracy | Sensitivity | Specificity | F1 |
|---|---:|---:|---:|---:|---:|
| Balanced RF | 0.906 | 0.841 | 0.863 | 0.828 | 0.807 |
| Elastic LR | 0.895 | 0.846 | 0.850 | 0.844 | 0.810 |
| Elastic+LGBM Blend | 0.901 | 0.793 | 0.638 | 0.891 | 0.703 |
| LightGBM | 0.902 | 0.837 | 0.838 | 0.836 | 0.798 |
| Logistic Reg. | 0.892 | 0.846 | 0.838 | 0.852 | 0.807 |
| MLP | 0.888 | 0.813 | 0.863 | 0.781 | 0.780 |
| Random Forest | **0.911** | 0.837 | **0.888** | 0.805 | 0.807 |
| SVM | 0.863 | 0.813 | 0.813 | 0.813 | 0.769 |

最佳时间外推测试 `AUC = 0.911`（Random Forest），多数模型 `AUC > 0.89`。相较 `3M`（最佳 `AUC = 0.857`），`6M` 可学习性显著更高——这符合临床直觉：随访时间越长，累积的治疗反应证据越充分，预测难度越低。

### A.2 6M ROC 曲线
![6M ROC 曲线](results/fixed_landmark_binary/ROC_6-Mo.png)

### A.3 6M 混淆矩阵
![6M 混淆矩阵](results/fixed_landmark_binary/CM_6-Mo.png)

### A.4 6M 校准图
![6M 校准图](results/fixed_landmark_binary/Calibration_6-Mo.png)

### A.5 6M 决策曲线分析
![6M DCA](results/fixed_landmark_binary/DCA_6-Mo.png)

### A.6 6M 阈值敏感性
![6M 阈值敏感性](results/fixed_landmark_binary/Threshold_Sensitivity_6-Mo.png)

### A.7 6M SHAP 解释
![6M SHAP 总图](results/fixed_landmark_binary/SHAP_6-Mo.png)

![6M SHAP 重要性条形图](results/fixed_landmark_binary/SHAP_6-Mo_Bar.png)

![6M SHAP 高风险个体](results/fixed_landmark_binary/SHAP_6-Mo_Local_HighRisk.png)

![6M SHAP 低风险个体](results/fixed_landmark_binary/SHAP_6-Mo_Local_LowRisk.png)

### A.8 临床解读

`6M` 结果的主要意义在于两点：

1. **验证早期反应信号的时间梯度**：`3M → 6M` 判别力单调递增，说明甲功恢复轨迹在治疗后半年内持续积累有效预测信息，符合 Graves 病放射性碘治疗后甲功逐步稳定的临床预期。
2. **支撑主模型 Global 分支的设计动机**：主模型的 Global Encoder 正是为了捕捉这种"随时间递增的纵向信号"而设计的——`6M` 的高 AUC 从侧面验证了纵向轨迹特征确实包含独立于 `3M` 快照之外的增量信息。
