...
将时间外推测试集按训练集患者级风险四分位分层后，`Q4` 的观察到复发率最高，且均值预测风险也最高。这里的观察到复发率误差条采用 Wilson `95%` 置信区间，因此这张图更接近论文中常见的“风险分层 + 不确定性”表达方式。
...
这张图强调的不是某一个时点的静态分类，而是**风险如何在随访过程中不断更新**。每一行是一个患者，每一列是一个随访窗口，颜色表示该窗口的复发风险；缺失窗口用浅灰显示。按患者级累计风险排序后，可以更直观看到高风险患者往往在多个窗口内持续维持较高风险，而不是只在单一窗口偶然抬升。
...
- 高风险真阳性：患者级累计风险最高且最终复发
- 低风险真阴性：患者级累计风险最低且最终未复发
- 边界病例：累计风险最接近正式阈值

每个病例左侧为 `FT3 / FT4 / TSH` 纵向轨迹，右侧为 `TeacherHead_Prob / Landmark_Signal / Final risk` 的同步变化。它的意义不在于“找一张好看的个案图”，而在于说明模型如何随着实验室与病程变化不断修正风险判断。
...
这张图回答的是一个更贴近分诊场景的问题：如果只优先盯住最高风险的前 `k%` 患者，能够提前覆盖多少真实复发个体。与 direct baseline 相比，当前正式模型在较低 `k` 区间内总体上能更快累积捕获复发病例，提示其患者级排序在临床资源有限时更有实用价值。

### 9.2 与 direct baseline 的患者级风险迁移
![与 direct baseline 的患者级风险迁移](results/relapse_teacher_frozen_fuse/TeacherFrozenFuse_Patient_Delta_vs_Direct.png)

散点图中，横轴为 direct baseline 的患者级风险，纵轴为当前正式模型的患者级风险，虚线为 `y=x` 参考线。位于参考线上方的点表示当前模型将该患者评估为更高风险，位于下方则表示风险被下调。它可以帮助识别：当前模型究竟把哪些病人重新分配到了更合理的风险层。
