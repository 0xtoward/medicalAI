...
## 一句话结论
在当前队列中，最值得作为主结论的不是“某个模型的最高分”，而是：**把问题定义为“RAI 后已经恢复 `Normal` 的患者，下一窗口是否会重新回到 `Hyper`”后，`repluse.py` 给出了最清晰、最贴近临床随访管理的结果。**
...
| 脚本 | 角色 | 主要输出目录 |
| --- | --- | --- |
| [`fixed_landmark_binary.py`](./fixed_landmark_binary.py) | 前导/基线分析：固定 `3M/6M` 二分类 | `results/fixed_landmark_binary/` |
| [`repluse.py`](./repluse.py) | 主线：rolling landmark 直接复发预警 | `results/repluse/` |
| [`repluse_recurrent_survival.py`](./repluse_recurrent_survival.py) | 扩展 1：recurrent-survival benchmark | `results/recurrent_survival/` |
| [`repluse_two_stage_physio.py`](./repluse_two_stage_physio.py) | 扩展 2：两阶段生理预测 | `results/repluse_two_stage_physio/` |
| [`fixed_landmark_multiclass.py`](./fixed_landmark_multiclass.py) | 补充分析：固定 landmark 三分类 | `results/fixed_landmark_multiclass/` |
...
## 主结果：rolling landmark 直接复发预警（`repluse.py`）
### 问题定义
...
![状态转移热图](results/repluse/Transition_Heatmaps.png)
...
在 `repluse.py` 的候选模型中，`Logistic Regression` 仍然是当前主线最均衡的模型：
...
![主线模型横向比较](results/repluse/Model_Comparison_Bar.png)
...
![多模型 hazard curve 对比](results/repluse/Hazard_Curve_Strict.png)
...
![interval-level 校准曲线](results/repluse/Calibration_Interval.png)
...
![主线 SHAP 图](results/repluse/Hazard_SHAP_Relapse.png)

![主线 LR 系数流图](results/repluse/Logistic_Reg_Coefficient_Flow.png)

结合 SHAP 与系数流图，当前主线最稳定的信号包括：
...
## 前导结果：固定 landmark 二分类（`fixed_landmark_binary.py`）
### 这个分支回答什么问题
...
- 最佳 `AUC`：`LightGBM = 0.909`
- 最佳 `PR-AUC`：`LightGBM = 0.859`
...
![6M 二分类 SHAP](results/fixed_landmark_binary/SHAP_6-Mo.png)

![6M Random Forest 代表树 anatomy](results/fixed_landmark_binary/Random_Forest_6-Mo_Forest_Anatomy.png)

从这些图看，`FT3_3M`、`FT3_6M`、`TSH_3M`、`TSH_6M` 和 `ThyroidW` 是最核心的分裂或贡献特征。这与临床直觉一致：固定时点的“当前是否仍甲亢”，本质上还是围绕当前激素状态和甲状腺负荷展开。
...
## 扩展分析 1：recurrent-survival benchmark（`repluse_recurrent_survival.py`）
### 这个分支的定位
...
| 最佳 next-window AUC | `Event-Specific RSF [history_augmented]` | `0.802` |
| 最佳 next-window PR-AUC | `AG Cox [history_augmented]` | `0.360` |
...
## 扩展分析 2：两阶段生理预测复发（`repluse_two_stage_physio.py`）
### 两阶段框架想解决什么
...
![Stage 1 生理预测比较](results/repluse_two_stage_physio/Stage1_Forecast_Bar.png)

![Stage 1 随机森林测试散点](results/repluse_two_stage_physio/RandomForest_Test_Scatter.png)
...
| --- | --- | --- | --- | --- |
| `direct` | `Elastic LR` | `0.822` | `0.323` | `0.067` |
| `predicted_only` | `Logistic Reg.` | `0.847` | `0.293` | `0.065` |
| `predicted_plus_current` | `Elastic LR` | `0.832` | `0.315` | `0.068` |

![两阶段分组比较](results/repluse_two_stage_physio/TwoStage_Group_Comparison.png)

![两阶段最佳组校准](results/repluse_two_stage_physio/Calibration_BestStage2.png)
...
## 补充分析：固定 landmark 三分类（`fixed_landmark_multiclass.py`）
这一分支把问题扩展成 `Hyper / Normal / Hypo` 三分类，用于门诊状态分流。

| Landmark | 模型 | 3分类 ACC | Hyper AUC | Hypo AUC |
| --- | --- | --- | --- | --- |
| `3M` | `Cascade RF` | `0.649` | `0.845` | `0.730` |
| `3M` | `Cascade GBDT` | `0.630` | `0.830` | `0.710` |
| `6M` | `Cascade RF` | `0.702` | `0.905` | `0.762` |
| `6M` | `Cascade GBDT` | `0.702` | `0.906` | `0.762` |
...
1. `fixed_landmark_binary.py` 先说明“当前状态识别”在 `3M/6M` 已经能做得不错。
2. `repluse.py` 再把问题提升到真正更有临床价值的层次：恢复正常后，下一窗口是否复发。
...
```bash
python fixed_landmark_binary.py
python repluse.py
python repluse_recurrent_survival.py
python repluse_two_stage_physio.py
python fixed_landmark_multiclass.py
```

### 清缓存重跑

```bash
python fixed_landmark_binary.py --clear-cache
python fixed_landmark_multiclass.py --clear-cache
python repluse.py --clear-cache
python repluse_recurrent_survival.py --clear-cache
python repluse_two_stage_physio.py --clear-cache
```

### 推荐优先阅读的结果目录

- `results/repluse/`：主线 rolling landmark 结果
- `results/fixed_landmark_binary/`：固定 landmark 二分类基线
- `results/recurrent_survival/`：recurrent-event benchmark
- `results/repluse_two_stage_physio/`：两阶段生理预测扩展
- `results/fixed_landmark_multiclass/`：三分类补充分析

### 主文优先阅读图表

1. `results/repluse/Transition_Heatmaps.png`
2. `results/repluse/Model_Comparison_Bar.png`
3. `results/repluse/Hazard_Curve_Strict.png`
4. `results/repluse/Calibration_Interval.png`
5. `results/repluse/Patient_Risk_Q1Q4.png`
6. `results/repluse/DCA_Patient.png`
7. `results/repluse/Hazard_SHAP_Relapse.png`
8. `results/repluse/Logistic_Reg_Coefficient_Flow.png`
9. `results/fixed_landmark_binary/ROC_6-Mo.png`
10. `results/recurrent_survival/Model_Comparison_Bar.png`
11. `results/repluse_two_stage_physio/TwoStage_Group_Comparison.png`
